const TikTok = {
  ...
  runChecks: function () {
    ...
    let reason = "";
    if (
      CONFIG.platformSettings.tt === "strict" &&
    ...
    if (this.isSafePage(path)) {
      ...
      reason = "safe page";
      UI.remove();
    } else if (Utils.isSessionAllowed("tt", warnScope) && !FocusState.isWork) {
      action = "allow";
      ...
      if (FocusState.isBreak) {
        ...
        CONFIG.platformSettings.tt === "W" ||
        CONFIG.platformSettings.tt === "warn"
      ) {
        action = "warn";
        reason = "warn mode";
        this.block(false, warnScope);
      } else {
        action = "allow";
    ...
    Utils.debugLog("tt", {
      ...
      sessionAllowed: Utils.isSessionAllowed("tt", warnScope),
      action,
      reason,
    });
  },
  ...
  block: function (isForced, warnScope) {
    if (!isForced && Utils.isSessionAllowed("tt", warnScope)) return;
    const mode = isForced ? "strict" : CONFIG.platformSettings.tt;
    UI.create(
      mode,
      "tt",
      () => {
        Utils.unlockVideo({ unmute: true });
        this.runChecks();
      },
