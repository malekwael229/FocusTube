...
function candidate() {
  ...
  for (const browser of ["chromium", "firefox"]) {
    const directory = path.join(buildRoot, "FocusTube-release-" + browser + "-v2.4.0");
    ...
    hashes["dist-release-builds/FocusTube-release-" + browser + "-v2.4.0.zip"] = digest(path.join(buildRoot, "FocusTube-release-" + browser + "-v2.4.0.zip"));
  }
  return hashes;
}
...
async function main() {
  ...
  try {
    for (const id of chosen) {
      ...
      const buildDir = path.join(root, "dist-release-builds", "FocusTube-release-" + (id === "firefox" ? "firefox" : "chromium") + "-v2.4.0");
      ...
      try {
        ...
        await report.case({ id: "extension-load", site: "extension", scope: "extension-ui", route: "popup.html", expected: "The exact 2.4.0 manifest is loaded" }, page, async () => {
          await page.goto(session.extensionURL + "popup.html");
          const manifest = await page.evaluate(() => chrome.runtime.getManifest());
          if (manifest.version !== "2.4.0") throw new Error("Wrong version loaded");
