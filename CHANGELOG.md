...
- Localized extension pages and extension-owned overlays automatically while keeping storage keys, runtime messages, alarm names, and other protocol identities stable. Arabic right-to-left handling stays scoped to extension-owned UI.
- Added deterministic catalog and localization contract checks, native Chromium Arabic and unsupported-locale fallback coverage, and package checks for all eight catalogs. Manual, live-site, native-speaker, and native Firefox localization validation remain pending.
