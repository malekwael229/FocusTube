const LinkedIn = {
    ...
    puzzlesOverlayId: 'ft-linkedin-puzzles-overlay',
    ...
    init: function () {
        ...
        chrome.storage.onChanged.addListener((changes) => {
            if (changes.platformSettings || changes.focusMode || changes.ft_timer_end || changes.ft_timer_type || changes.hide_li_feed || changes.hide_li_puzzles || changes.hide_li_addfeed) {
                this.runChecks();
            }
        });
    ...
    runChecks: function () {
        ...
        if (CONFIG.visualHiding.liFeed && !feedAllowed) {
        ...
        if (CONFIG.visualHiding.liPuzzles || CONFIG.visualHiding.liAddFeed) {
    ...
    showFeedOverlay: function (isForced) {
        ...
        feedColumn.style.position = 'relative';
        feedColumn.style.overflow = 'hidden';
        feedColumn.style.maxHeight = '400px';
    ...
    showSidebarOverlays: function (isForced) {
        ...
        if (CONFIG.visualHiding.liPuzzles) {
            const existing = document.getElementById(this.puzzlesOverlayId);
            if (existing && (existing.dataset.ftDismiss === 'true') !== allowDismiss) {
                existing.remove();
            }
            if (!document.getElementById(this.puzzlesOverlayId)) {
                const puzzlesCard = this.findSidebarCard("Today's puzzles");
                if (puzzlesCard && !puzzlesCard.dataset.ftDismissed) {
                    puzzlesCard.style.position = 'relative';
                    puzzlesCard.style.overflow = 'hidden';
                    const overlay = this.createSmallOverlay(this.puzzlesOverlayId, 'Hidden', allowDismiss, puzzlesCard);
                    puzzlesCard.appendChild(overlay);
                }
            }
        } else {
            const existing = document.getElementById(this.puzzlesOverlayId);
        ...
        if (CONFIG.visualHiding.liAddFeed) {
            ...
            if (!document.getElementById(this.addFeedOverlayId)) {
                ...
                if (addFeedCard && !addFeedCard.dataset.ftDismissed) {
                    addFeedCard.style.position = 'relative';
                    addFeedCard.style.overflow = 'hidden';
    ...
    removeSidebarOverlays: function () {
        const puzzles = document.getElementById(this.puzzlesOverlayId);
        const addFeed = document.getElementById(this.addFeedOverlayId);
        if (puzzles) puzzles.remove();
        if (addFeed) addFeed.remove();
    },
